...
/**
 * Defines methods related to repackaging the files that were build based on one of the supported frameworks, or
 * the WAR files.
 */
public class AppFramework {
  ...
  /**
   * Checks if the file is build based on supported framework, or is it a WAR file.
   * If the file is not JAR or WAR, the error is logged.
   *
   * @param mainFile file that is being checked
   * @return FrameworkSupport object having methods needed to properly copy JAR entries with or without
   *         prefix specific to framework
